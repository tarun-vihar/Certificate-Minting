...
export class VerifyCertificateComponent implements OnInit, OnDestroy {
  ...
  public filteredUniversities: ReplaySubject<University[]> = new ReplaySubject<University[]>(1);
  ...
  filterBanks() {
    ...
    this.filteredUniversities.next(
      this.universities.filter(university => university.universityName.toLowerCase().indexOf(search) > -1)
    );
  }
  ...
  isInvalidControl(controlName: string): boolean{
    ...
    const isInvalid =  (this.certificateDetailsForm.controls[controlName].touched || this.certificateDetailsForm.controls[controlName].touched) && this.certificateDetailsForm.controls[controlName].invalid;
    ...
    if(isInvalid && this.certificateDetailsForm.controls[controlName].errors){
      switch(true){
        case this.certificateDetailsForm.controls[controlName].errors.required:
          ...
          !this.errorMessages[controlName].includes(errMessage) && this.errorMessages[controlName].push(errMessage);
      }
    } else {
